...
## 3. Relationship to Ledger (Repository, RFC-0003)
...
1.  **Lifecycle Evaluation:** The Lifecycle engine evaluates the incoming Intent against the transition matrix and state authority rules.
2.  **CAS Execution:** Lifecycle requests a state mutation from the Repository using Compare-And-Swap.
3.  **Ledger Commit:** The Repository validates the expected version, mutates the state, and appends the entry to the Ledger.
4.  **History Materialization:** The Ledger commit triggers synchronous materialization of the corresponding History entry.
5.  **Proof & Telemetry Generation:** Cryptographic proof generation and telemetry recording are initiated following the history update.
